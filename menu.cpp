...
int MENU::show(String title, String options[], int noptions) {
  ...
  if (Lpointer != pointer) { //checks if the pointer has changed, to see if it is needed to draw again
    ...
    for (int i = 0; i < noptions; i++) {
      ...
      screen::centerText((screen::height - noptions * 10) / 2 + i * 10, opt, BLACK);
    }
    ...
    for (int i = 0; i < noptions; i++) {
      ...
      screen::centerText((screen::height - noptions * 10) / 2 + i * 10, opt, YELLOW);
    }
    Lpointer = pointer;
  }

  if (controls::getbutton("DOWN") && millis() - lastmil > 200) {
  ...
  if (controls::getbutton("UP") && millis() - lastmil > 200) {
